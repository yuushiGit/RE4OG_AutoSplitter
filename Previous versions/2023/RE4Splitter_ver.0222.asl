...
state("bio4", "1.1.0")
{
    ...
    float xPos            : 0x8706CC; // X Position
    float yPos            : 0x8706EC; // Y Position
    float zPos            : 0x8706DC; // Z Position
}
...
update
{
    ...
    var componentCoords = vars.updateTextComponent("Coords");
    componentFrames.Text2 = vars.loadRemovalFrames.ToString();
    componentPause.Text2 = vars.isPaused.ToString();
    componentCoords.Text2 = string.Format("X: {0} Y: {1} Z: {2}", current.xPos, current.yPos, current.zPos);
}
