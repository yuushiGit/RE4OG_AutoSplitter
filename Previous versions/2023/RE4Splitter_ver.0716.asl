...
state("bio4", "1.1.0")
{
    ...
    bool isLoading        : 0x858F77; // Loading Screen
    bool isSubtitle       : 0x817840; // Subtitles on the screen
    bool isMGEnd          : 0x8594BB; // Ending Cutscene in Main Game
    bool isSWEnd          : 0x851173; // Ending Cutscene in Separate Ways
    bool isEvent          : 0x867C09; // Events and Animations
    ...
    byte frameRate        : 0x82B7A0; // FPS Settings (30 or 60)
}
...
// Screen States (Stage Loading: 1, Room Loading: 2, Gameplay: 3, Door Demo: 4, Options Menu: 6)
vars.screenStates = new Dictionary<string, byte>()
    {
        { "Stage Loading", 1 },
        { "Room Loading", 2 },
        { "Gameplay", 3 },
...
// Menu Screens (Inventory: 1, Map: 2, Picked Up: 4, Merchant: 16, Contact: 32, File: 64, Key Item: 128)
vars.menus = new Dictionary<string, byte>()
    {
        { "Not Opened", 0 },
        { "Inventory", 1 },
        { "Map", 2 },
...
// Characters (Leon: 0, Ashley: 1, Ada: 2, Hunk: 3, Krauser: 4, Wesker: 5)
...
// Room IDs at the start of the game
vars.startRooms = new Dictionary<string, short>()
    {
        { "MG", 256 },  // Footpath to the Village
        { "SW", 1280 }, // Village Center
        { "AA", 288 }   // Main Menu (Wharf A)
    };

// Room IDs at the end of the game
vars.endRooms = new Dictionary<string, short>()
    {
        { "MG", 819 },  // Underground Water Vein
        { "SW", 1310 }, // Steel Tower
        { "AA", 1038 }  // Communications Tower
    };
...
// Show Inventory Time
if ((current.menu == vars.menus["Inventory"] || current.menu == vars.menus["Key Item"]) && settings["ShowInvTime"])
{
    // Update the text
    ...
    // Start the timer
    if (current.screenTransition < vars.threshold && old.screenTransition > vars.threshold)
    {
...
if (settings["ShowInvCount"])
{
    ...
    if ((current.menu == vars.menus["Inventory"] || current.menu == vars.menus["Key Item"]) && old.menu == vars.menus["Not Opened"])
    {
        vars.totalInvCount++;
...
if (!current.isLoading && current.screenState == vars.screenStates["Gameplay"])
{
    vars.isPaused = false;
...
// Show Log
var componentDebug = vars.updateTextComponent("Debug");
componentDebug.Text2 = string.Format("Paused: {0} Frames: {1}", vars.isPaused, vars.loadRemovalFrames);
}

start
{
    // Check to see if Main Game is running
    if (current.screenTransition < vars.threshold && old.screenTransition > vars.threshold && vars.characters[current.character] == "Leon" && current.room == vars.startRooms["MG"] && settings["MainGameSplits"])
    {
...
if (current.menu == vars.menus["Not Opened"] && old.menu == vars.menus["Map"] && vars.characters[current.character] == "Ada" && current.room == vars.startRooms["SW"] && settings["SeparateWaysSplits"])
{
...
if (!current.isSubtitle && old.isSubtitle && vars.characters[current.character] == "Ada" && current.room == vars.startRooms["AA"] && settings["AssignmentAdaSplits"])
{
...
// Final Splits
if (current.isMGEnd && !old.isMGEnd && current.room == vars.endRooms["MG"] && vars.gameMode == vars.gameModes["MG"]                                                                                                 // Main Game Ending
    || current.isSWEnd && !old.isSWEnd && current.room == vars.endRooms["SW"] && vars.gameMode == vars.gameModes["SW"]                                                                                              // Separate Ways Ending
    || !current.isSubtitle && old.isSubtitle && current.isEvent && current.choice == 66048 && vars.obtainedPlagaSamples.Count == 5 && current.room == vars.endRooms["AA"] && vars.gameMode == vars.gameModes["AA"]) // Assignment Ada Ending
{
    return true;
}
}
