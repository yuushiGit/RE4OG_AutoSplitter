...
state("bio4", "1.1.0")
{
    ...
    bool subtitle         : 0x817840; // Subtitle on the screen (true | false)
    ...
    bool screenTrans      : 0x858F77; // Screen Transition (true | false)
    byte screenState      : 0x85A780; // Screen State (StageInit: 1, RoomInit: 2, MainLoop: 3, Doordemo: 4, Option: 6)
    byte screenUpdate     : 0x85A7B5; // Screen Update (Processing: 8, Stopping: 12 & 28)
...
state("bio4", "1.0.6")
{
    ...
    bool screenTrans      : 0x8556F7;
    byte screenState      : 0x856F00;
    byte screenUpdate     : 0x856F35;
...
update
{
    ...
    if ((!current.screenTrans || current.subtitle) && current.screenState == 3 && current.screenUpdate != 12 && current.screenUpdate != 28)
        vars.elapsedFrames += current.totalFrames - old.totalFrames;

    var componentFrames = vars.updateTextComponent("Frames");
    var componentScreen = vars.updateTextComponent("Screen");
    componentFrames.Text2 = string.Format("{0}", vars.elapsedFrames);
    componentScreen.Text2 = string.Format("Trans: {0} State: {1} Update: {2}", current.screenTrans, current.screenState, current.screenUpdate);
