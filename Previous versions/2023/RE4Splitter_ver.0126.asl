...
update
{
    if (!current.isCommand && old.isCommand && current.isEvent && !current.isCutscene && !current.isMiniCutscene && !old.isSubtitle)
    {
        vars.isUnskippableEvent = true;
    }
    else if ((!current.isEvent && old.isEvent || current.isCutscene && !old.isCutscene || current.isSubtitle && !old.isSubtitle) && current.screenState == 3 && vars.isUnskippableEvent)
    {
        vars.isUnskippableEvent = false;
    }
    ...
    componentCommand.Text2 = string.Format("command: {0} bool: {1}", current.isCommand, vars.isUnskippableEvent);
