...
state("bio4", "1.1.0")
{
    ...
    byte screenState      : 0x8597BB; // Screen States
...
update
{
    ...
    // Show Inventory Time
    if ((current.menu == 1 || current.menu == 128) && settings["ShowInvTime"])
    {
        // Update the text
        ...
        // Stop the timer
        else if (current.isLoading && !old.isLoading)
...
start
{
    ...
    if (current.screenTransition <= 240 && old.screenTransition >= 240 && current.room == 256 && current.character == 0 && settings["MainGameSplits"])
    {
...
isLoading
{
    ...
    return current.isLoading || current.screenTransition >= 240 || current.screenState == 208;
}
