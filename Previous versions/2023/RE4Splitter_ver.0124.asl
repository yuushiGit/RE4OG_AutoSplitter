...
state("bio4", "1.1.0")
{
    byte gameMode         : 0x85F725; // Game Modes (MG: 0, SW: 2, AA: 3)
    short room            : 0x85A788; // Room IDs
    byte menu             : 0x87AD04; // Menu Screens (Inv: 1, Map: 2, Merchant: 16, Contact: 32, Manual: 64, Key Item Selection: 128)
    ...
    bool isCommand        : 0x817948; // Commands displayed at bottom of screen (true | false)
    bool isScrTrans       : 0x858F77; // Screen Transition (true | false)
    byte scrTransRate     : 0x858F7B; // Screen Transition Rate (1 - 255)
    byte screenState      : 0x85A780; // Screen States (StageInit: 1, RoomInit: 2, MainLoop: 3, Doordemo: 4, Option: 6)
...
state("bio4", "1.0.6")
{
    ...
    bool isEvent          : 0x867C09;
    bool isCutscene       : 0x803C5F;
    bool isMiniCutscene   : 0x867C0D;
    bool isCommand        : 0x817948;
...
{
    /*
    if (current.isEvent && !old.isEvent && current.command != 81 && !current.isCutscene && !current.isMiniCutscene)
    {
        vars.isUnskippableEvent = true;
    }
    else if ((!current.isEvent && old.isEvent || current.isSubtitle && !old.isSubtitle) && current.screenState == 3 && vars.isUnskippableEvent)
    {
        vars.isUnskippableEvent = false;
    }

    // Calculation of elapsed frames
    if ((current.scrTransRate != 255 || !current.isScrTrans || current.isSubtitle) && current.screenState == 3 && !vars.isUnskippableEvent)
    {
    ...
    */
    ...
    if ((current.scrTransRate != 255 || !current.isScrTrans || current.isSubtitle)
        && (!current.isEvent || current.isSubtitle || current.isCutscene || current.isMiniCutscene || current.isContact || current.menu != 0)
    ...
    var componentCommand = vars.updateTextComponent("Command");
    ...
    componentCommand.Text2 = string.Format("{0}", current.isCommand);
    componentFlags.Text2 = string.Format("Event: {0} Cut: {1} Mini: {2}", current.isEvent, current.isCutscene, current.isMiniCutscene);
...
split
{
    ...
    if (current.isMGEnd && !old.isMGEnd && current.room == 819 && vars.gameMode == vars.gameModes["MG"]                                                                                                  // Main Game Ending
        ...
        || !current.isSubtitle && old.isSubtitle && current.choice == 66048 && current.isEvent && vars.obtainedPlagaSamples.Count == 5 && current.room == 1038 && vars.gameMode == vars.gameModes["AA"]) // Assignment Ada Ending
    {
        return true;
    }
