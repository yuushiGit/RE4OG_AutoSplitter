...
init
{
    ...
    vars.convertFramesToTime = (Func<long, TimeSpan>) ((frames) => {
        return TimeSpan.FromMilliseconds(frames * (1000.0 / current.frameRate));
    });
...
update
{
    ...
    // Show Inventory Time
    if ((current.menu == 1 || current.menu == 128) && !current.isScrTrans && settings["ShowInvTime"])
    {
        ...
        componentInvTime.Text2 = vars.convertFramesToTime(vars.inventoryFrames).ToString("hh\\:mm\\:ss\\.ff");
    }
...
gameTime
{
    ...
    return vars.convertFramesToTime(vars.loadRemovalFrames);
}
