...
state("bio4", "1.1.0")
{
    ...
    byte menu              : 0x87AD04; // Menu Screen (Inv: 1, Map: 2, Merchant: 16, Key Item Selection: 128)
    byte menuExit          : 0x858F97; // Exit the Menu (ON: 63 | 64)
    ...
    byte stageId           : 0x85F70D; // Stage ID (1: Village, 2: Castle, 3: Island, 4: AA and Mercs, 5: SW)
...
init
{
    ...
    Action ResetVariables = () =>
    {
        ...
        vars.dropFrames = 0; // Count this when the game is paused
...
update
{
    ...
    else if (current.screenTransA <= 240 && old.screenTransA >= 240 && current.menu == 0 && vars.isMenuScreen)
...
isLoading
{
    // Pause timer when the Loading Screen, Pause Menu, Game is paused
    bool isGameplayLoading = (current.loading == 255 || current.screenTransA >= 240) && !vars.isMenuScreen; // Gameplay Loading
    bool isMenuLoading = (current.screenTransB == 2 || current.menuExit != 0) && vars.isMenuScreen; // Menu Loading
    bool isCutsceneLoading = current.cutscene == 12 || current.cutscene == 28; // Cutscene Loading
    bool isPauseMenu = current.pauseMenu == 208; // Pause Menu
    bool isGamePaused = current.gameState == old.gameState && vars.dropFrames >= 2; // Game is paused

    vars.isTimerPaused = isGameplayLoading || isMenuLoading || isCutsceneLoading || isPauseMenu || isGamePaused;
    return vars.isTimerPaused;
}
