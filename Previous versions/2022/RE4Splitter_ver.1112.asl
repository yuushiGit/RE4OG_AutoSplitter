...
update
{
    ...
    if ((current.loading == 255 || current.screenTrans >= 240)
       || ((current.cutscene == 12 || current.cutscene == 28) && current.screenTrans != 0)
       || (current.pauseMenu == 7 && current.menu == 0)
       || (vars.dropFrames > 1 && current.frameRate == 60 || vars.dropFrames > 2 && current.frameRate == 30))
    {
...
isLoading
{
    ...
    // Frame Drops
    if (vars.dropFrames > 1 && current.frameRate == 60 || vars.dropFrames > 2 && current.frameRate == 30)
        return true;
