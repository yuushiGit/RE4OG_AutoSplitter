state("bio4")
{
    ...
    byte swEnding          : 0x851171; // Ending Cutscene in Separate Ways
...
init
{
    ...
    vars.specificAreas.Add(260, 180);
    vars.specificAreas.Add(283, 180);
    vars.specificAreas.Add(533, 180);
    vars.specificAreas.Add(555, 180);
    vars.specificAreas.Add(768, 180);
    vars.specificAreas.Add(790, 180);
    vars.specificAreas.Add(817, 180);
    vars.specificAreas.Add(1288, 180);
    vars.specificAreas.Add(1310, 180);
}
...
update
{
    ...
    vars.isAreaLoading = (current.loadingScreen == 255 || current.screenTrans >= 240) && current.menu != 1;
    vars.isInvLoading = current.screenTrans == 255 && current.menu == 1;
    vars.isCutSceneLoading = vars.specificAreas.ContainsKey(current.currentArea) && current.screenTrans >= vars.specificAreas[current.currentArea] && current.cutScene == 64;
...
start
{
    ...
    else if (current.menu == 0 && old.menu == 2 && current.currentArea == 1280 && settings["SeparateWaysSplits"])
    {
        vars.ResetVariables();
        vars.gameMode = "SW";
        vars.ObtainedKeyItems = new List<byte> { };
...
split
{
    ...
    // Final Splits
    return
    current.mgEnding != old.mgEnding && current.currentArea == 819 && vars.gameMode == "MG" ||
    current.swEnding != old.swEnding && current.currentArea == 1310 && vars.gameMode == "SW" ||
    current.aaEnding != old.aaEnding && current.currentArea == 1038 && vars.gameMode == "AA";
}
