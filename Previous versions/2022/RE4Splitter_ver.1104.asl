...
state("bio4", "1.1.0")
{
    ...
    byte pauseMenu         : 0x8597BB; // Pause Menu (ON: 208)
    ...
    int gameState          : 0xE3F920; // Increases constantly while the game is running
    int gameplayState      : 0x8666FF; // Increases constantly while the gameplay
...
state("bio4", "1.0.6")
{
    ...
    int roomId             : 0x856F08;
    int igt                : 0x85BE84;
    int gameState          : 0xE3C0A0;
    int gameplayState      : 0x862E7F;
...
startup
{
    ...
    settings.Add("Infos", false, "Information");
        ...
        settings.Add("ShowInvCounter", true, "Inventory Counter", "Infos");
...
init
{
    ...
    vars.ResetVariables = (Action) (() =>
    {
        ...
        vars.isPauseMenu = false; // This will be true if the pause menu is opening
        vars.menuState = 0; // Store the value of gameplayState when the menu screen is opened
        ...
        vars.ObtainedKeyItems = new List<byte>(); // Initialize the list of Key Items obtained
        vars.ObtainedPlagaSamples = new List<byte>(); // Initialize the list of Plaga Samples obtained
        vars.BlackListedDoors[0] = Tuple.Create(0, 0); // Initialize BlackListedDoors[0]
        vars.BlackListedDoors[1] = Tuple.Create(0, 0); // Initialize BlackListedDoors[1]
    });
    vars.ResetVariables();
}
...
{
    // Save the menuState
    if (current.menu != 0 && old.menu == 0 || current.pauseMenu == 208 && old.pauseMenu == 0)
        vars.menuState = current.gameplayState;
    ...
    // Check to see if the pause menu is opening
    if (current.pauseMenu == 208 && old.pauseMenu == 0 && !vars.isPauseMenu)
        vars.isPauseMenu = true;
    else if (current.gameplayState != vars.menuState && vars.isPauseMenu)
    ...
    {
        dynamic componentHealthValue = vars.UpdateTextComponent("Health");
        if (current.stageId < 4)
            componentHealthValue.Text2 = "Leon: " + current.mainHealth.ToString() + " Ashley: " + current.ashleyHealth.ToString();
        else
            componentHealthValue.Text2 = "Ada: " + current.mainHealth.ToString();
    }
    ...
    if (settings["ShowInvCounter"])
    {
        dynamic componentInvCount = vars.UpdateTextComponent("Invs");
        ...
        if (vars.chapterInvs != oldChapterInvs)
            componentInvCount.Text2 = "Total: " + vars.totalInvs.ToString() + " Chapter: " + vars.chapterInvs.ToString();
    }
}
...
split
{
    ...
    if (current.roomId == 518 && old.roomId == 527 && current.chapter == 10 && vars.gameMode == "MG" && settings["DoorSplits"])
        return true;
...
isLoading
{
    ...
    // Menu Loading
    else if (current.screenTrans == 255 && current.gameplayState == vars.menuState && vars.isMenuScreen)
        return true;
