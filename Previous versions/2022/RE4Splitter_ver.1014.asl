...
update
{
    ...
    vars.isInvLoading = (current.loading == 255 || current.screenTrans == 255 && current.menuOpen == 1) && vars.isInventory;
    vars.isCutScene = current.cutScene == 12 || current.cutScene == 28;
    vars.isPauseMenu = current.pauseMenu == 208;
    vars.isNowLoading = (vars.isRoomLoading || vars.isInvLoading || vars.isCutScene || vars.isPauseMenu) && vars.gameMode != "Default";
