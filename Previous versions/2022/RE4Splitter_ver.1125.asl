...
state("bio4", "1.1.0")
{
    ...
    byte screenState       : 0x85A780; // Screen State (Loading: 1 and 2, Gameplay: 3, Fadeout: 4, Pause Menu: 6)
    byte cutscene          : 0x85A7B5; // Cutscene Screen (MG and AA: 12, SW: 28)
    byte frameRate         : 0x82B7A0; // FPS Setting (30 or 60)
    int room               : 0x85A788; // Room ID
...
init
{
    ...
    vars.ResetVariables = (Action<string>) ((gameMode) =>
    {
        ...
        vars.isNowLoading = false; // Set this to true during the loading
    ...
    vars.BlackListedDoors = new List<Tuple<int, int>>()
    {
        ...
        Tuple.Create(778, 790), // Communications Tower (Cutscene) ↔ Machine Room Entry
...
update
{
    // Check to see if you are on the loading screen
    if ((current.screenState == 4 || current.loading == 255) && !vars.isNowLoading)
        vars.isNowLoading = true;
    else if (current.screenTrans <= 240 && old.screenTrans >= 240 && vars.isNowLoading)
    ...
    if (vars.isNowLoading)
        componentTimerPhase.Text2 = "Main Loading";
    else if ((current.cutscene == 12 || current.cutscene == 28) && current.screenTrans != 0)
        componentTimerPhase.Text2 = "Cutscene Loading";
    else if (current.screenState == 6)
        componentTimerPhase.Text2 = "Pause Menu";
    else if (vars.dropFrames > 2 && current.frameRate == 60 || vars.dropFrames > 3 && current.frameRate == 30)
        componentTimerPhase.Text2 = "Frame Drop";
    else
        componentTimerPhase.Text2 = "Unpaused";

    dynamic componentDropFrames = vars.UpdateTextComponent("DropFrames");
    componentDropFrames.Text2 = "" + vars.dropFrames.ToString();
}
...
start
{
    ...
    if (current.subtitle == 0 && old.subtitle == 1 && (current.room == 1029 || current.room == 288) && current.character == 2 && settings["AssignmentAdaSplits"])
    {
...
{
    // Door Splits
    if (current.room != old.room && current.room <= 1310 && vars.gameMode != "Default" && settings["DoorSplits"])
    {
        ...
        print("CompletedDoors: " + vars.CompletedDoors[vars.CompletedDoors.Count - 1]);
        return true;
    }
