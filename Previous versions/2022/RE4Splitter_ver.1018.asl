state("bio4")
{
    ...
    byte loadStart         : 0x858F77; // Loading Start
    byte loadFinish        : 0x851075; // Loading Finish
...
state("bio4", "1.0.6")
{
    ...
    byte loadStart         : 0x8556F7;
    byte loadFinish        : 0x84D7F5;
    byte menu              : 0x877484;
...
update
{
    // Check to see if the Menu Screen is open
    if (current.menu != 0 && old.menu == 0 && !vars.isMenuScreen)
        vars.isMenuScreen = true;
    else if (current.menuExit == 0 && old.menuExit != 0 && vars.isMenuScreen)
        vars.isMenuScreen = false;
    ...
    if (current.menu == 1 && old.menu == 0 && componentInvCounter != null)
    {
        vars.invCount++;
...
isLoading
{
    ...
    bool isGameplayLoading = (current.loadStart == 255 || current.loadFinish == 255) && !vars.isMenuScreen;
    bool isMenuLoading = (current.screenTrans == 255 || current.menuExit != 0) && vars.isMenuScreen;
    bool isCutsceneLoading = current.cutscene == 12 || current.cutscene == 28;
    bool isPauseMenu = current.pauseMenu == 208;
    return isGameplayLoading || isMenuLoading || isCutsceneLoading || isPauseMenu;
}
