state("bio4")
{
    ...
    byte cutScene          : 0x8597BB; // Cutscene Screen
...
init
{
    ...
    vars.LongBlackScreenDoors = new Dictionary<int, int> { };
    ...
    // Specific Doors with long Black Screens
    vars.LongBlackScreenDoors.Add(257, 100);
    vars.LongBlackScreenDoors.Add(519, 100);
    vars.LongBlackScreenDoors.Add(553, 100);
    vars.LongBlackScreenDoors.Add(260, 180);
    vars.LongBlackScreenDoors.Add(283, 180);
    vars.LongBlackScreenDoors.Add(533, 180);
    vars.LongBlackScreenDoors.Add(555, 180);
    vars.LongBlackScreenDoors.Add(768, 180);
    vars.LongBlackScreenDoors.Add(790, 180);
    vars.LongBlackScreenDoors.Add(817, 180);
    vars.LongBlackScreenDoors.Add(1288, 180);
    vars.LongBlackScreenDoors.Add(1310, 180);
}
...
update
{
    ...
    vars.isCutSceneLoading = vars.LongBlackScreenDoors.ContainsKey(current.roomId) && current.screenTrans >= vars.LongBlackScreenDoors[current.roomId] && current.cutScene == 64;
    vars.isOptionsMenu = current.cutScene == 208;

    vars.isRoomChange = current.roomId != old.roomId && vars.gameMode != "Default";
    ...
    // Call Frame Counter
    if (vars.isRoomLoading || vars.isInvLoading || vars.isCutSceneLoading || vars.isOptionsMenu)
        vars.totalPauseFrames += current.totalFrames - old.totalFrames;

    if (vars.isRoomChange || vars.isMGEnding || vars.isSWEnding || vars.isAAEnding)
        vars.FrameCounter();
...
isLoading
{
    // Pause Timer when the Loading Screen or Options Menu
    if (current.menu == 1)
    {
    ...
    {
        if (vars.isRoomLoading || vars.isCutSceneLoading)
        {
            vars.StopWatch.Start();
            return true;
        }
    ...
    /*
    if (current.menu == 1) return vars.isInvLoading;
    else return vars.isRoomLoading || vars.isCutSceneLoading || vars.isOptionsMenu;
    */
}
...
gameTime
{
    ...
    if (vars.isRoomChange || vars.isMGEnding || vars.isSWEnding || vars.isAAEnding)
    {
        if (current.fpsSetting == 60)
            return TimeSpan.FromMilliseconds(vars.calcFrames * (double) 1000 / 60);
        else if (current.fpsSetting == 30)
            return TimeSpan.FromMilliseconds(vars.calcFrames * (double) 1000 / 30);
    }
}
...
{
    // Door Splits
    if (vars.isRoomChange && settings["DoorSplits"])
    {
    ...
    // Final Splits
    return vars.isMGEnding || vars.isSWEnding || vars.isAAEnding;
}
