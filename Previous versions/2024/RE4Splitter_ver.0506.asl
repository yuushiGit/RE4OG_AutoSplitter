...
state("bio4", "1.1.0")
{
    ...
    byte endOfChapter     : 0x85979A; // End Of Chapter Screen
...
update
{
    ...
    if ((current.endOfChapter == 128 && !current.isSubtitle || (current.isMovie || current.isSWMovie) && !current.isQTE || current.isCutscene || current.contact != 0) && current.isEvent)
    {
    ...
    if (current.screenState != 3 || current.screenTransition == 2 && current.menu != 0 || (current.endOfChapter == 128 && !current.isSubtitle || (current.isMovie || current.isSWMovie) && !current.isQTE || current.isCutscene || current.contact != 0) && current.isEvent)
    {
...
isLoading
{
    ...
    // Menu Loads
    if (current.screenTransition == 2 && current.menu != 0)
    {
        return true;
    }
