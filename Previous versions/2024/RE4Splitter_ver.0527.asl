...
init
{
    ...
    vars.resetVariables = (Action<string>)((gameMode) =>
    {
        ...
        vars.completedDoors = new List<Tuple<short, short>>(); // Store the rooms passed
        vars.playedCutscenes = new List<string>();             // Store the cutscenes played
        vars.obtainedKeyItems = new List<string>();            // Store the key items obtained
...
update
{
    ...
    // Cutscenes
    bool isCutscenes = ((current.isCutscene || current.isMovie) && !current.isQTE || current.isMiniCutscene || current.contact != 0) && current.isEvent;

    // Add load removal frames
    if (!isDoorLoads && !isCutscenes)
    {
    ...
    // if (Keyboard.GetKeyStates(Key.Return) && KeyStates.Down > 0 && settings["SoftReset"])
    // {
...
split
{
    ...
    // Event Splits
    String cutsceneId = current.room.ToString() + current.cutscene.Substring(4);
    ...
    String movieId = current.room.ToString() + current.movie.Substring(4);
