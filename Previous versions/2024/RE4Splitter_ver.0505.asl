...
state("bio4", "1.1.0")
{
    ...
    bool isMovie          : 0x803C5F; // Movies in Main Game
    bool isOldMovie       : 0x86CD44; // Movies in Separate Ways
...
update
{
    ...
    // Debug
    var componentCutsceneTime = vars.updateTextComponent("Cutscene Time");
    var componentIsPause = vars.updateTextComponent("IsPause");
    componentCutsceneTime.Text2 = vars.cutsceneTime.Elapsed.ToString("hh\\:mm\\:ss\\.ff");

    if (current.isEvent && (current.isCutscene || current.isMovie || current.isOldMovie || current.contact != 0))
    {
    ...
    if (current.screenState != 3 || current.screenTransition == 2 && current.menu != 0 || current.isEvent && (current.isCutscene || current.isMovie || current.isOldMovie || current.contact != 0))
    {
        componentIsPause.Text2 = "True";
    }
    else
    {
        componentIsPause.Text2 = "False";
    }
}
...
split
{
    ...
    // Final Splits
    if (current.isMGEnding && !old.isMGEnding && current.room == 819 && vars.gameMode == vars.gameModes["MG"]                                                                                            // Main Game Ending
        || current.isSWEnding && !old.isSWEnding && current.room == 1310 && vars.gameMode == vars.gameModes["SW"]                                                                                        // Separate Ways Ending
...
isLoading
{
    return current.screenState != 3 || current.screenTransition == 2 && current.menu != 0 || current.isEvent && (current.isCutscene || current.isMovie || current.isOldMovie || current.contact != 0);
}
