...
update
{
    ...
    if (current.isEvent && !current.isSubtitle && current.menu == 0 || current.menu == 32)
    {
    ...
    if (current.screenState != 3 || current.screenTransition == 2 && current.menu != 0 || current.isEvent && !current.isSubtitle && current.menu == 0 || current.menu == 32)
    {
...
start
{
    ...
    if (!current.isEvent && old.isEvent && vars.characters[current.character] == "Leon" && current.room == 256 && settings["MainGameSplits"])
    {
...
isLoading
{
    return current.screenState != 3 || current.screenTransition == 2 && current.menu != 0 || current.isEvent && !current.isSubtitle && current.menu == 0 || current.menu == 32;
}
